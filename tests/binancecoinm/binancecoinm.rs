...
use venues::binance::coinm::{
    AccountTradesRequest, ExchangeInfoRequest, GetAccountRequest, NewOrderRequest,
    OrderSide, OrderType, PrivateRestClient, PublicRestClient, QueryOrderRequest,
    RateLimiter, TimeInForce, CancelOrderRequest,
};
...
use tokio::time::sleep;
    ...
    async fn get_current_ask_price(_public_client: Arc<PublicRestClient>, _symbol: &str) -> Result<f64> {
    ...
    async fn place_buy_order(
        ...
        println!("Placed buy order: ID {}, status: {:?}", order_resp.data.order_id, order_resp.data.status);
    ...
    async fn verify_trade_execution(
        ...
        if !trades_resp.data.is_empty() {
            println!("Found {} trade(s) for order {}", trades_resp.data.len(), order_id);
    ...
    async fn test_stable_coin_buy_order_flow() -> Result<()> {
        ...
        if initial_balance < 10.0 {
            return Err(anyhow!("Insufficient balance for testing (need at least 10 USDT)"));
        }
        ...
        let mut order_id = place_buy_order(private_client.clone(), &symbol, quantity, current_price).await?;
        ...
        while iteration < max_iterations {
            iteration += 1;
            println!("Iteration {}/{}: Waiting for order execution...", iteration, max_iterations);
            ...
            match order_status.as_str() {
                ...
                "CANCELED" | "REJECTED" | "EXPIRED" => {
                    return Err(anyhow!("Order {} was {} unexpectedly", order_id, order_status));
                }
                "TIMEOUT" | _ => {
                    if iteration < max_iterations {
                        ...
                        current_price *= 1.005;
                        order_id = place_buy_order(private_client.clone(), &symbol, quantity, current_price).await?;
                    } else {
                        println!("Max iterations reached, canceling final order...");
                        cancel_order(private_client.clone(), &symbol, order_id).await?;
                        return Err(anyhow!("Order not filled after {} iterations", max_iterations));
                    }
                }
            }
        }
        ...
        if trade_found {
            ...
            println!("⚠️  No trades found for order {} (may be due to timing)", order_id);
        }
        ...
        println!("Final {} balance: {} (change: {})", 
            base_asset, final_balance, final_balance - initial_balance);
    ...
    async fn test_exchange_info_access() -> Result<()> {
        ...
        assert!(!symbol.is_empty(), "Should find at least one stable coin pair");
        println!("✅ Exchange info access test passed, found symbol: {}", symbol);
        Ok(())
    }
    ...
    fn test_order_quantity_calculation() {
        ...
        assert!(quantity >= min_qty, "Quantity should be at least the minimum");
        assert!(quantity <= balance * 0.1 / price, "Quantity should not exceed 10% of balance");
