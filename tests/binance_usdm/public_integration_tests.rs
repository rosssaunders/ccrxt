...
async fn test_ping() {
    ...
    println!("Ping successful: request took {:?}", response.request_duration);
}
...
async fn test_get_server_time() {
    ...
    println!("Server time: {} (took {:?})", response.data.server_time, response.request_duration);
}
...
async fn test_get_exchange_info() {
    ...
    assert!(!response.data.rate_limits.is_empty(), "Should have rate limits");
    ...
    assert!(!symbol.base_asset.is_empty(), "Base asset should not be empty");
    // margin_asset is now an enum, so we just verify it exists (which it does if deserialization succeeded)
    
    println!("Exchange info fetched successfully: {} symbols, took {:?}", 
             response.data.symbols.len(), response.request_duration);
...
async fn test_basic_endpoints() {
    ...
    if let Ok(funding_response) = funding_result {
        assert!(!funding_response.data.is_empty(), "Should have funding rate data");
...
async fn test_rate_limiting() {
    ...
    assert!(successful_count >= 3, "At least 3 out of 5 requests should succeed");
    
    println!("Rate limiting test: {}/5 requests succeeded", successful_count);
}
...
async fn test_endpoint_diversity() {
    ...
    let endpoints_tested = vec![
        ...
        ("funding_rate_info", client.get_funding_rate_info().await.is_ok()),
    ];

    let successful_endpoints: Vec<_> = endpoints_tested.iter()
    ...
    assert!(successful_endpoints.len() >= 3, 
           "At least 3 endpoints should succeed, got: {:?}", successful_endpoints);
}
