...
fn discover_venue_public_methods(
    venues_src_path: &Path,
    venue_name: &str,
) -> Result<Vec<EndpointMethod>, Box<dyn std::error::Error>> {
    ...
    for public_rest_path in public_rest_paths {
        if public_rest_path.exists() {
            methods.extend(extract_methods_from_directory(
                &public_rest_path,
                venue_name,
            )?);
        }
...
fn extract_methods_from_directory(
    rest_path: &Path,
    venue_name: &str,
) -> Result<Vec<EndpointMethod>, Box<dyn std::error::Error>> {
    let mut methods = Vec::new();
...
fn extract_endpoint_method(
    content: &str,
    venue_name: &str,
    _file_name: &str,
) -> Option<EndpointMethod> {
...
fn generate_test_file_content(venue_name: &str, methods: &[EndpointMethod]) -> String {
    ...
    content.push_str(&format!(
        "    let client = create_{}_client().await;\n",
        venue_name
    ));
    content.push_str("    \n");
...
fn generate_client_creation_function(venue_name: &str) -> String {
    ...
    format!(
        ...
        venue_name, rate_limiter_path, base_url
    )
}
