...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match client.get_underlying(swap_request).await {
        Ok(response) => {
            ...
            if let Some(data) = response.data.first() {
                println!("Found {} underlying assets for SWAP instruments:", data.uly.len());
    ...
    match client.get_underlying(futures_request).await {
        Ok(response) => {
            if let Some(data) = response.data.first() {
                println!("Found {} underlying assets for FUTURES instruments:", data.uly.len());
                
                // Show BTC-related underlying assets
                let btc_underlying: Vec<_> = data.uly.iter()
                    .filter(|u| u.contains("BTC"))
                    .collect();
                ...
                let eth_underlying: Vec<_> = data.uly.iter()
    ...
    match client.get_underlying(option_request).await {
        Ok(response) => {
            if let Some(data) = response.data.first() {
                println!("Found {} underlying assets for OPTION instruments:", data.uly.len());
