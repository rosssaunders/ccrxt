...
async fn maintain_orderbook(
    rest_client: &BinanceCoinMPublicRest,
    symbol: &str,
    metrics: &mut Metrics,
    ...
    while !subscription_confirmed {
        if let Ok(Some(msg)) = tokio::time::timeout(
            ...
            return Err(Box::new(OrderBookError::Timeout("Subscription confirmation timeout".into())));
        }
    }
    ...
    while let Some(update) = message_buffer.pop_front() {
        ...
        println!("Processing update - U: {}, u: {}, pu: {}, last_update_id: {}", 
                update.first_update_id, update.final_update_id, 
                update.previous_final_update_id, last_update_id);
        ...
        if !first_message_processed {
            ...
            if update.first_update_id > last_update_id + 1 || update.final_update_id < last_update_id + 1 {
                ...
                return Err(Box::new(OrderBookError::SequenceGap(format!(
            ...
            if update.previous_final_update_id != last_update_id {
                metrics.gaps_detected += 1;
                return Err(Box::new(OrderBookError::SequenceGap(format!(
                    "Gap in sequence: previous_final_id({}) != last_update_id({})",
                    update.previous_final_update_id, last_update_id
                ))));
            }
        }
