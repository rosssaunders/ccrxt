...
use venues::binance::coinm::{
    BinanceCoinMPrivateRest, BinanceCoinMAPIError,
};
...
fn create_client(testnet: bool) -> Result<Arc<BinanceCoinMPrivateRest>> {
    dotenv().ok();
    let api_key = env::var("API_KEY").expect("API_KEY not set");
    let api_secret = env::var("API_SECRET").expect("API_SECRET not set");

    let client = if testnet {
        BinanceCoinMPrivateRest::new(
...
async fn main() -> Result<()> {
    let cli = Cli::parse();
    let client = create_client(cli.testnet)?;
