...
impl RestClient {
    ...
    pub async fn get_all_orders(&self, params: GetAllOrdersRequest) -> AllOrdersResult<Vec<AllOrder>> {
        ...
        let mut query_pairs = serde_urlencoded::to_string(&params).map_err(|e| AllOrdersError::Other(format!("Failed to serialize params: {e}")))?;
        ...
        let resp = execute_request::<Vec<AllOrder>>(&self.client, &full_url, method, Some(headers), None)
            .await
            .map_err(|e| match e {
                crate::binance::usdm::Errors::ApiError(api_err) => AllOrdersError::Other(format!("API error: {api_err}")),
                crate::binance::usdm::Errors::HttpError(http_err) => AllOrdersError::Other(format!("HTTP error: {http_err}")),
                crate::binance::usdm::Errors::Error(msg) => AllOrdersError::Other(msg),
                crate::binance::usdm::Errors::InvalidApiKey() => AllOrdersError::InvalidKey("Invalid API key or signature".to_string()),
            })?;
