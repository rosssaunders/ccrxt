#[cfg(test)]
mod tests {
    use crate::binance::shared::client::ResponseHeaders as SharedResponseHeaders;
    use crate::binance::usdm::{ResponseHeaders, RateLimitHeader};
    use crate::binance::usdm::rate_limit::{RateLimitHeaderKind, IntervalUnit};
    ...
    fn test_response_headers_from_shared_with_invalid_values() {
        ...
        let mut shared_headers = HashMap::new();
        shared_headers.insert("x-mbx-used-weight-1m".to_string(), "not-a-number".to_string());
        shared_headers.insert("x-mbx-order-count-10s".to_string(), "5".to_string());
