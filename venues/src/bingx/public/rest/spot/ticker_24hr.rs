...
mod tests {
    ...
    fn test_ticker_24hr_deserialization() {
        ...
        assert_eq!(ticker.bid_price, Some(44999.0));
        assert_eq!(ticker.bid_qty, Some(1.5));
        assert_eq!(ticker.ask_price, Some(45001.0));
        assert_eq!(ticker.ask_qty, Some(2.0));
    ...
    async fn test_get_24hr_ticker_method_exists() {
        ...
        // The result can be either success or failure depending on network/test environment
        let _result = client.get_24hr_ticker(&request).await;
        // Just verify the method exists and returns a Result - don't assert on success/failure
    }
}
