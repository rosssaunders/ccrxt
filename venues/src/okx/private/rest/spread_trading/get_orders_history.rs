...
const GET_SPREAD_ORDERS_HISTORY_ENDPOINT: &str = "/api/v5/sprd/orders-history";

/// Request parameters for getting spread orders history
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct GetSpreadOrdersHistoryRequest {
    /// Spread ID
    ...
    /// Date older than 7 days will be truncated.
...
/// Response data for getting spread orders history
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct HistorySpreadOrderData {
    /// Spread ID
    #[serde(rename = "sprdId")]
    pub sprd_id: String,
...
impl RestClient {
    /// Get spread orders history
    ///
    /// Retrieve the completed order data for the last 21 days
    ///
    /// [docs](https://www.okx.com/docs-v5/en/#spread-trading-rest-api-get-orders-last-21-days)
    pub async fn get_spread_orders_history(
        &self,
        request: Option<GetSpreadOrdersHistoryRequest>,
    ) -> RestResult<HistorySpreadOrderData> {
        self.send_get_request(
            GET_SPREAD_ORDERS_HISTORY_ENDPOINT,
            request.as_ref(),
...
mod tests {
    ...
    #[test]
    fn test_get_spread_orders_history_request_full() {
        let request = GetSpreadOrdersHistoryRequest {
        ...
        let deserialized: GetSpreadOrdersHistoryRequest =
            serde_json::from_str(&serialized).unwrap();
        assert_eq!(request, deserialized);
    }

    #[test]
    fn test_get_spread_orders_history_request_minimal() {
        let request = GetSpreadOrdersHistoryRequest {
            sprd_id: None,
            ord_type: None,
            state: None,
    ...
    #[test]
    fn test_get_spread_orders_history_request_none() {
        let request: Option<GetSpreadOrdersHistoryRequest> = None;
        assert!(request.is_none());
    }

    #[test]
    fn test_history_spread_order_data_deserialization() {
        let json_response = r#"{
        ...
        let order: HistorySpreadOrderData = serde_json::from_str(json_response).unwrap();
    ...
    #[test]
    fn test_history_spread_order_data_serialization() {
        let order = HistorySpreadOrderData {
        ...
        let deserialized: HistorySpreadOrderData = serde_json::from_str(&serialized).unwrap();
        assert_eq!(order, deserialized);
    }

    #[test]
    fn test_history_order_states() {
    ...
    fn test_history_time_filters() {
        ...
        assert!(serialized.contains("\"end\":\"1597112783085\""));
    }
}
