...
/// Request parameters for getting historical mark price candlesticks
...
/// Response for getting historical mark price candlesticks
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GetHistoryMarkPriceCandlesResponse {
    ...
    /// Response message
    pub msg: String,
    /// Historical mark price candlestick data. Each entry is [ts,o,h,l,c,confirm]
    pub data: Vec<HistoryMarkPriceCandle>,
}
...
impl RestClient {
    ...
    ///
    /// # Arguments
    /// * `request` - The historical mark price candlesticks request parameters
    ///
    /// # Returns
    /// Response containing the list of historical mark price candlesticks
...
mod tests {
    ...
    #[test]
    fn test_get_history_mark_price_candles_request_structure() {
    ...
    #[test]
    fn test_get_history_mark_price_candles_request_with_pagination() {
        let request = GetHistoryMarkPriceCandlesRequest {
    ...
    #[test]
    fn test_history_mark_price_candle_structure() {
        ...
        let candle: HistoryMarkPriceCandle = serde_json::from_value(candle_json).unwrap();
        assert_eq!(candle.0, "1597026383085"); // ts
        assert_eq!(candle.1, "45000.0");       // o
        assert_eq!(candle.2, "46100.5");       // h
        assert_eq!(candle.3, "44900.0");       // l
        assert_eq!(candle.4, "45850.25");      // c
        assert_eq!(candle.5, "1");             // confirm
    }

    #[test]
    fn test_get_history_mark_price_candles_response_structure() {
        ...
        assert_eq!(first_candle.1, "45000.0");
        assert_eq!(first_candle.2, "46100.5");
        assert_eq!(first_candle.3, "44900.0");
        assert_eq!(first_candle.4, "45850.25");
    ...
    #[test]
    fn test_history_mark_price_candles_serialization_roundtrip() {
    ...
    #[test]
    fn test_history_mark_price_candles_minimal_request() {
        let request = GetHistoryMarkPriceCandlesRequest {
            inst_id: "BTC-USD-SWAP".to_string(),
            after: None,
            before: None,
    ...
    #[test]
    fn test_history_mark_price_candle_serialization_roundtrip() {
        let original = HistoryMarkPriceCandle(
        ...
        let serialized = serde_json::to_value(&original).unwrap();
        let deserialized: HistoryMarkPriceCandle = serde_json::from_value(serialized).unwrap();
    ...
    #[test]
    fn test_utc_time_bar_formats() {
        let request = GetHistoryMarkPriceCandlesRequest {
            inst_id: "BTC-USD-SWAP".to_string(),
            after: None,
            before: None,
            bar: Some("1Dutc".to_string()),
            limit: None,
        };
