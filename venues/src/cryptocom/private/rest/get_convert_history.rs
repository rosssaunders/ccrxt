...
impl RestClient {
    ...
    ///
    /// See: <https://exchange-docs.crypto.com/exchange/index.html#private-staking-get-convert-history>
    ///
    ...
    pub async fn get_convert_history(
        ...
        let signature = self.sign_request("private/staking/get-convert-history", id, &params, nonce)?;
        ...
        let response = self.client
            .post(&format!("{}/v1/private/staking/get-convert-history", self.base_url))
            .json(&request_body)
            .send()
            .await?;
...
mod tests {
    ...
    fn test_convert_history_final_statuses() {
        let statuses = vec!["COMPLETED", "INSUFFICIENT_BALANCE", "SLIPPAGE_TOO_HIGH", "MARKET_CLOSED"];
