        ...
        T: DeserializeOwned,
    {
        ...
        let query_string = if let Some(params) = query {
            serde_urlencoded::to_string(params)
                .map_err(|e| crate::gateio::unified::GateIoError::Internal(e.to_string()))?
        } else {
            String::new()
        };
        ...
        let body_str = if let Some(body_data) = body {
            serde_json::to_string(body_data).map_err(crate::gateio::unified::GateIoError::Json)?
        } else {
            String::new()
        };
        ...
        let response = self.http_client
            ...
            .await
            .map_err(|e| crate::gateio::unified::GateIoError::Network(format!("HTTP request failed: {}", e)))?;
        ...
        let response_text = response
            .text()
            .map_err(|e| crate::gateio::unified::GateIoError::Network(format!("Failed to read response: {}", e)))?;

        if status >= 200 && status < 300 {
            let data: T = serde_json::from_str(&response_text)
                .map_err(crate::gateio::unified::GateIoError::Json)?;
            Ok(data)
        } else {
            let error: crate::gateio::unified::errors::ErrorResponse =
