...
use crate::gateio::perpetual::{Result, rate_limit::RateLimiter};
        ...
        T: DeserializeOwned,
    {
        ...
        let query_string = if let Some(params) = query {
            serde_urlencoded::to_string(params)
                .map_err(|e| crate::gateio::perpetual::GateIoError::Internal(e.to_string()))?
        } else {
            String::new()
        };
        ...
        let body_str = if let Some(body_data) = body {
            serde_json::to_string(body_data).map_err(crate::gateio::perpetual::GateIoError::Json)?
        } else {
            String::new()
        };
