        ...
        Q: Serialize,
    {
        ...
        let full_url = if let Some(params) = query {
            let query_string = serde_urlencoded::to_string(params).map_err(|e| {
                crate::gateio::delivery::GateIoError::InvalidParameter(format!(
        ...
        let response = self.http_client.execute(request).await.map_err(|e| {
            crate::gateio::delivery::GateIoError::Network(format!("HTTP request failed: {}", e))
        })?;
        ...
        let body = response.text().map_err(|e| {
            crate::gateio::delivery::GateIoError::Network(format!("Failed to read response: {}", e))
        })?;

        if (200..300).contains(&status) {
            let data: T = serde_json::from_str(&body)
                .map_err(|e| crate::gateio::delivery::GateIoError::Json(e))?;
            Ok(data)
        } else {
            let error: crate::gateio::delivery::errors::ErrorResponse = serde_json::from_str(&body)
                .map_err(|e| crate::gateio::delivery::GateIoError::Json(e))?;
