...
use crate::gateio::spot::{Result, rate_limit::RateLimiter};
...
impl RestClient {
    ...
    pub fn new(api_key: String, api_secret: String, testnet: bool) -> Result<Self> {
        let client = Client::builder()
            ...
            .build()
            .map_err(crate::gateio::spot::GateIoError::Http)?;
        ...
        T: DeserializeOwned,
    {
        ...
        let _permit = self.rate_limiter.get_permit(endpoint).await.map_err(|_| {
            crate::gateio::spot::GateIoError::RateLimitExceeded {
        ...
        let query_string = if let Some(params) = query {
            serde_urlencoded::to_string(params)
                .map_err(|e| crate::gateio::spot::GateIoError::Internal(e.to_string()))?
        } else {
            String::new()
        };
        ...
        let body_str = if let Some(body_data) = body {
            serde_json::to_string(body_data).map_err(crate::gateio::spot::GateIoError::Json)?
        } else {
            String::new()
        };
        ...
        if status.is_success() {
            let data: T = serde_json::from_str(&response_text)
                .map_err(crate::gateio::spot::GateIoError::Json)?;
            Ok(data)
        } else {
