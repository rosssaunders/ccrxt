...
impl RestClient {
    ...
    pub async fn get_options_candlesticks(&self, params: OptionsCandlesticksRequest) -> crate::gateio::Result<Vec<OptionsCandlestick>> {
        self.get_with_query("/options/candlesticks", Some(&params)).await
    }
    
    /// Mark price candlesticks of an underlying
    ///
    /// Retrieves mark price candlestick data for an underlying asset.
    pub async fn get_underlying_candlesticks(&self, params: UnderlyingCandlesticksRequest) -> crate::gateio::Result<Vec<UnderlyingCandlestick>> {
        self.get_with_query("/options/underlying/candlesticks", Some(&params)).await
    }
}