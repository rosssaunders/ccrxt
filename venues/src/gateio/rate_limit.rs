...
use super::{errors::GateIoError, rate_limiter_trait::GateIoRateLimiter};
...
impl RateLimiter {
    ...
    fn categorize_endpoint(&self, endpoint: &str) -> String {
        match endpoint {
            ...
            // Account endpoints (treat similar to wallet)
            endpoint if endpoint.starts_with("/account/") => "wallet".to_string(),
            // Margin endpoints
...
impl GateIoRateLimiter for RateLimiter {
    async fn get_permit(&self, endpoint: &str) -> Result<(), GateIoError> {
        let _permit = self.get_permit_internal(endpoint).await.map_err(|_| {
            GateIoError::RateLimitExceeded {
    ...
    async fn update_from_headers(&self, headers: &HashMap<String, String>, endpoint: &str) {
        ...
        let _status = self.update_from_headers(&rate_limit_header, endpoint);
    }
