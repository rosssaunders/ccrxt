use venues::coinbase::advanced_trade::websocket::authenticated::CoinbaseAdvancedTradeWebSocketAuthenticated;
use futures::StreamExt;

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Example 1: Basic authenticated connection and user channel subscription
    async fn basic_auth_example(jwt: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn futures_balance_example(jwt: String) -> Result<(), Box<dyn std::error::Error>> {
        let mut ws = CoinbaseAdvancedTradeWebSocketAuthenticated::new(jwt);
        ws.connect().await?;
    ...
    async fn complete_workflow_example(jwt: String) -> Result<(), Box<dyn std::error::Error>> {
        let mut ws = CoinbaseAdvancedTradeWebSocketAuthenticated::new(jwt);
        ws.connect().await?;
    ...
    async fn error_handling_example(jwt: String) -> Result<(), Box<dyn std::error::Error>> {
        let mut ws = CoinbaseAdvancedTradeWebSocketAuthenticated::new(jwt);
        ...
        while let Some(message) = stream.next().await {
            match message {
                ...
                Err(e) => {
                    ...
                    match e.downcast_ref::<venues::coinbase::advanced_trade::error::CoinbaseAdvancedTradeError>() {
                        Some(venues::coinbase::advanced_trade::error::CoinbaseAdvancedTradeError::WebSocketError(_)) => {
    ...
    let jwt = "your_jwt_token_here".to_string();
