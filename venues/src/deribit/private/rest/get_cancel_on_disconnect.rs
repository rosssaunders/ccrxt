...
pub struct GetCancelOnDisconnectResponse {
    ...
    pub jsonrpc: String,
    /// Cancel On Disconnect configuration result
    pub result: CancelOnDisconnectResult,
}
...
impl RestClient {
    ...
    pub async fn get_cancel_on_disconnect(&self, scope: Option<CancelOnDisconnectScope>) -> RestResult<GetCancelOnDisconnectResponse> {
        let request = GetCancelOnDisconnectRequest { scope };
...
mod tests {
    ...
    fn test_response_structures_deserialization_enabled() {
        ...
        let response: GetCancelOnDisconnectResponse = serde_json::from_value(response_json).unwrap();
